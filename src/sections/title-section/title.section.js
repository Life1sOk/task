...
const TitleSection = ({ id, state, edit, editState }) => {
    ...
    return (
        <TitleContainer>
            ...
            {
                id ? <h2>Товар id: {id}</h2> :
                    edit ? <h2>Товар id: {edit}</h2> :
            ...
            }
            {
                !state ?
                    <StateContainer>
                        ...
                        <select onChange={(e) => stateChangeHandler(e)} defaultValue={editState} name="product-state" id="product-state">
